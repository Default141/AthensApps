...
public class DBMethod {
    ...
    public String[] itemCombo(){
       ...
       // dbExecuteQuery(sql);
        ...
        ArrayList<HashMap> all = db.queryRows(sql);
        String[] item = new String[product.size()];
        int i = 0;
        for(HashMap t : all){
            String name = (String)t.get("product_name");
            String type = (String)t.get("product_type");
            String locate = (String)t.get("product_locate");
            String supplierId = (String)t.get("product_supplierId");
            double amount = (double)t.get("product_amount");
            int price = (Integer)t.get("product_price");
            product.add(new DAOproduct(name, type, locate, price, price, amount));
            item[i++] = name;
        }
        dbDisConnect();
